...
CRPlot is for constant resistance mode: sets end voltage, sets constant resistance, reads from instrument,
...
class CRPlot:
    ...
    def _init_(self, instrument, date_time, constant_resistance, end_voltage):
        """ main method in CCPlot that prepares for reading data from instrument and writing data into database """
        self.constant_resistance = float(constant_resistance)
        self.end_voltage = float(end_voltage)
        self.time_in_sec = 0
        self.time_in_hour = 0
        self.ampere_hour = 0
        self.milli_amphour = 0
        self.watt_hour = 0
        ...
        #start program
        if_continue = self.cr_create_table()
        if if_continue:
            self.cr_set_instrument()
            self.inst.input_on()
            ...
            #display ending ui
            end_ui(df, 'Constant Resistance', self.initial_voltage, self.end_voltage, date_time, self.time_in_sec)
    ...
    def cr_close_db(self):
        """ closes connection to sqlite database """
        self.conn.commit()
        self.conn.close()

    def cr_set_instrument(self):
        """ reset instrument """
        self.inst.reset()
        self.inst.constant_resistance_mode()
        self.inst.set_resistance(str(self.constant_resistance))
